...
import me.trent.listener.API.VoteListenerAPI;
import me.trent.skyblock.commands.CommandImplementation;
import me.trent.skyblock.events.*;
import me.trent.skyblock.guis.*;
import me.trent.skyblock.island.Island;
import me.trent.skyblock.island.IslandUtils;
import me.trent.skyblock.nms.ReflectionManager;
...
import me.trent.skyblock.commands.IslandCommands;
...
import me.trent.skyblock.island.quests.Quests;
import me.trent.skyblock.island.quests.Requirement;
import me.trent.skyblock.island.rules.RuleUI;
import me.trent.skyblock.island.scoreboards.IslandBoard;
...
import me.trent.skyblock.placeholderAddons.PAPIExpansion;
...
public class SkyBlock extends JavaPlugin {
    ...
    private VoteListenerAPI voteListenerAPI;
    ...
    public VoteListenerAPI getVoteListenerAPI() {
        return voteListenerAPI;
    }
    ...
    public void onEnable(){
        ...
        voteListenerAPI = new VoteListenerAPI();

        MultiMaterials.setupMultiversionMaterials();

        if (!setupEconomy()){
            Bukkit.getLogger().log(Level.SEVERE,  "          --- Trents_SkyBlock ---");
            Bukkit.getLogger().log(Level.SEVERE, "Error: Could not find Vault...");
            Bukkit.getLogger().log(Level.SEVERE, "Disabling Trents_SkyBlock");
        ...
        Storage.scoreboard_worlds = getFileManager().getScoreboard().fetchStringList("scoreboard-worlds");
        ...
        for (Player p : Bukkit.getOnlinePlayers()){
            if (!SkyBlock.getInstance().getUtils().hasMemoryPlayer(p.getUniqueId())){
                SkyBlock.getInstance().getUtils().loadPlayer(p.getUniqueId(), false);
            }
        }
        ...
        CommandImplementation commandExecutors = new CommandImplementation();
        commandExecutors.registerCustomCommands(); // register them...
                    ...
                    if (getUtils().hasMemoryPlayer(p.getUniqueId())) {
                        ...
                        if (island != null){
                            ...
                            if (is != null && getReflectionManager().nmsHandler != null){
                                if (getInstance().getReflectionManager() != null){
                                    if (getInstance().getReflectionManager().nmsHandler != null){
                            ...
                            //island is null, we probably want to send the client a new worldborder packet...
                            SkyBlock.getInstance().getReflectionManager().nmsHandler.sendBorder(p, p.getLocation().getBlockX()*2, p.getLocation().getBlockZ()*2, 2);
                        }
                    }
                }
            }
